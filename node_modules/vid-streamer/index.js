/*!
 * Toan Vnaking custome & Develop from VidStreamer.js, big thanks to Andrew Weeks
...
var settings = require('./config/vidStreamer.json');
...
var vidStreamer = function(req, res) {
    ...
    if (reqUrl.search == '?V') {
        reqUrl.pathname = reqUrl.pathname + '.mp4';
    }
    ...
    if (settings.random) {
        if (!randomFile(info)) {
            handler.emit("noRandomFiles", res, {
                message: info.path
            });
            return false;
        }
    ...
    try {
        ...
        if (info.mime == 'image/jpeg') {
            info.path = settings.rootFolder + settings.badfileImage;
            info.file = settings.badfileImage;
            stat = fs.statSync(info.path);
        } else {
